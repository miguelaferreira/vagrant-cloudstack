...
describe CloudstackNetworkingService do
  ...
  let(:service) { CloudstackNetworkingService.new(cloudstack_compute, ui) }
  ...
  context 'when the API call succeeds' do
    ...
    describe '#enable_static_nat' do
      it 'enables static nat' do
        machine = Mash.new({:id => 'id', :data_dir => Pathname.new('dir')})
        ip = CloudstackResource.new('some ip address id', 'name', 'public_ip')

        expect(service).to receive(:save_ip_address_to_data_dir)
        service.enable_static_nat(machine, ip)
      end
    end
    ...
    describe '#create_port_forwarding_rule' do
      it 'creates a port forwarding rule' do
        ...
        service.create_port_forwarding_rule(machine, rule, ip)
      end
    end
    ...
    describe '#create_firewall_rule' do
      it 'creates a firewall rule' do
        ...
        service.create_firewall_rule(machine, rule, ip)
      end
    end
  end
  ...
  context 'when the API call does not succeed' do
    ...
    describe '#enable_static_nat' do
      it 'raises an API failure error' do
        ...
        expect { service.enable_static_nat(machine, ip) }.to raise_exception(ApiCommandFailed, 'some error message')
      end
    end
  end
end
