  ...
  module Cloudstack
    module Service
      class CloudstackNetworkingService < BaseService
        ...
        attr_accessor :network_resource,
                      :networking_config,
                      :resource_service

        def enable_static_nat(machine, ip_address)
          options = {
          ...
          begin
            ...
            save_ip_address_to_data_dir(machine, ip_address.id)
          rescue Fog::Compute::Cloudstack::Error => e
            raise Errors::FogError, :message => e.message
          end
        end

        def create_port_forwarding_rule(machine, rule, ip_address)
          options = {
              :ipaddressid      => ip_address.id,
          ...
          begin
            ...
            save_port_forwarding_to_data_dir(machine, port_forwarding_rule['id'])
          rescue Fog::Compute::Cloudstack::Error => e
            raise Errors::FogError, :message => e.message
          end
        end

        def create_firewall_rule(machine, rule, ip_address)
          options = {
          ...
          begin
            ...
            save_firewall_rule_to_data_dir(machine, firewall_rule['id'])
          rescue Fog::Compute::Cloudstack::Error => e
        ...
        def save_firewall_rule_to_data_dir(machine, rule_id)
          save_element_to_data_dir(machine, 'firewall', rule_id)
        end

        def save_port_forwarding_to_data_dir(machine, rule_id)
          save_element_to_data_dir(machine, 'port_forwarding', rule_id)
        end

        def save_ip_address_to_data_dir(machine, ip_address_id)
          save_element_to_data_dir(machine, 'static_nat', ip_address_id)
        end

        def save_element_to_data_dir(machine, element_name, value)
          element_file = machine.data_dir.join(element_name)
          element_file.open('a+') do |f|
            f.write("#{value}\n")
            f.close()
          end
          @ui.debug("File written to data dir: #{element_file}")
        end
      end
    end
  end
